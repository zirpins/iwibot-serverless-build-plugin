 ...
 */
export class NodeJsModuleBundler {
  modules: IModule[];
  ...
  async bundle ({ include = [], exclude = [], deepExclude = [] }: {
    ...
    const readModule = async ({ packagePath, packageDir, relativePath, packageJson }) => {
      const filter = (dirPath, stats) => {
        ...
        const endParts = dirPath.split(testPackagePath)[1].split(sep).slice(-2);
  ...
  private resolveDependencies (
    ...
    const recurse = (packageDir, _include = [], _exclude = []) => {
      const jsonObj = require(join(packageDir, 'package.json')); // eslint-disable-line
      const { name } = jsonObj;
      ...
      Object.keys(dependencies).map((packageName) => {
        ...
        if (_include.length && !(_include.indexOf(packageName) >= 0 || _include.indexOf('**/*') > -1)) { return; }
        ...
        const childPackageJsonPath = join(nextPackagePath, 'node_modules', 'package.json');
        let childJsonObj;
        ...
        resolvedDeps.push({ ...childResult, packageDir, relativePath, packageJson: childJsonObj });
      });
